...
Roofset::Roofset() {
	...
	_texture = textures_manager->getTexture(L"assets\\tex\\roofset.png");
	_overhangTexture = textures_manager->getTexture(L"assets\\tex\\roofset_overhang.png");

	_animations = std::make_shared<Animations>(L"roofset", _texture, sf::Vector2i(32, 32), _texture->getSize().y / 32, _texture->getSize().x / 32);
	...
	std::shared_ptr<Group> mulch_roof = std::make_shared<Group>(L"mulch roof");
	std::shared_ptr<Group> mud_roof = std::make_shared<Group>(L"mud roof");
	//std::shared_ptr<Group> brick_roof = std::make_shared<Group>(L"brick roof");
	//std::shared_ptr<Group> fortified_roof = std::make_shared<Group>(L"fortified roof");

	_groups.push_back(red_roof);
	_groups.push_back(stone_roof);
	_groups.push_back(mulch_roof);
	_groups.push_back(mud_roof);
	...
	for (int i = 0; i < _groups.size(); i++) {
		...
		std::shared_ptr<FlatRoofPartPrefab> prefab =
			std::make_shared<FlatRoofPartPrefab>(
				_groups[i]->name,
				_animations,
				sf::Vector2i(0, 0),
				nullptr,
...
std::shared_ptr<FlatRoofPartPrefab> Roofset::getPrefab(int id) {
...
std::shared_ptr<Roofset> roofset = nullptr;