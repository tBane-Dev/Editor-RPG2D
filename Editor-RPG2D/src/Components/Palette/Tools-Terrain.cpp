...
ToolTerrainButton::ToolTerrainButton(std::shared_ptr<Texture> texture, std::shared_ptr<Texture> hoverTexture, std::shared_ptr<Texture> pressTexture, std::shared_ptr<Texture> toolTexture, sf::Vector2i position) : ButtonWithSprite(texture, hoverTexture, pressTexture, nullptr, position) {
	_toolTexture = toolTexture;
}

ToolTerrainButton::~ToolTerrainButton() {

}

void ToolTerrainButton::cursorHover() {
	if (_toolTexture)
		ButtonWithSprite::cursorHover();
}

void ToolTerrainButton::handleEvent(const sf::Event& event) {
	if (_toolTexture)
		ButtonWithSprite::handleEvent(event);
}

void ToolTerrainButton::update() {
	ButtonWithSprite::update();
}

void ToolTerrainButton::draw() {
...
ToolsTerrain::ToolsTerrain() : Tools() {
	...
	_toolType = ToolTerrainType::None;
}

ToolsTerrain::~ToolsTerrain() {

}

void ToolsTerrain::addTool(std::shared_ptr<Texture> texture, std::shared_ptr<Texture> hoverTexture, std::shared_ptr<Texture> pressTexture, std::shared_ptr<Texture> toolTexture, std::function<void()> function) {
	std::shared_ptr<ToolTerrainButton> toolButton = std::make_shared<ToolTerrainButton>(texture, hoverTexture, pressTexture, toolTexture);
	toolButton->_onclick_func = function;
	_tools.push_back(toolButton);
}

void ToolsTerrain::setTool(std::shared_ptr<ToolTerrainButton> button, ToolTerrainType type) {
...
void ToolsTerrain::setPosition(sf::Vector2i position) {
...
void ToolsTerrain::handleEvent(const sf::Event& event) {
