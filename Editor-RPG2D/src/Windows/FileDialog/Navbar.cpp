...
NavBar::NavBar(sf::Vector2i position, sf::Vector2i size) {
	...
	_items.clear();
	std::vector<std::filesystem::path> location = { 
		L"",
		L"C:\\", 
		L"C:\\Users",
		L"C:\\Users\\tBane" 
	...
	for (auto& loc : location) {
		// directory item
		_items.push_back(std::make_shared<NavBarItem>(loc, position + sf::Vector2i(posX, (_rect.size.y - small_text_rect_height) / 2)));
		posX += _items.back()->getSize().x;
	}
}
...
void NavBar::add(std::filesystem::path path) {
	int posX = 0;
	if (!_items.empty()) {
		posX = _items.back()->getPosition().x + _items.back()->getSize().x;
	}
	_items.push_back(std::make_shared<NavBarItem>(path, _rect.position + sf::Vector2i(posX, (_rect.size.y - small_text_rect_height) / 2)));
}
